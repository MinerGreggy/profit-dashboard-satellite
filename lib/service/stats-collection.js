...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'profit-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('profit').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('profit')[service]);
